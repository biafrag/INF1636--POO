...
public class Cavalo extends Peca {
	...
	public Vector<Pair> CatchPossibleMovements(int x, int y) {
		...
		if(i+1<8 && j+2<8) 
		{
			...
			if(TabuleiroFacade.getTFacade().TemPecaIndice(i+1,j+2)==false)
			{
		...
		if(i+2<8 && j+1<8)
		{
			...
			if(TabuleiroFacade.getTFacade().TemPecaIndice(i+2,j+1)==false)
			{
	...
	public Vector<Pair> PossibleEats(int x, int y) {
		...
		if(i+1<8 && j+2<8)
		{
			if(TabuleiroFacade.getTFacade().TemPecaIndice(i+1,j+2) && TabuleiroFacade.getTFacade().getPecaCor(i+1, j+2)!=color)
			{
		...
		if(i+2<8 && j+1<8)
		{
			if(TabuleiroFacade.getTFacade().TemPecaIndice(i+2,j+1) && TabuleiroFacade.getTFacade().getPecaCor(i+2, j+1)!=color)
			{
